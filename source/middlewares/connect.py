...
class Connect():
    ...
    def Login(self) -> None:
        ...
        self.cursor = bd.connect(f'DRIVER={self.driver};SERVER={self.server};UID={self.username};PWD={self.password};DATABASE={self.database}').cursor()
    ...
    def LoginAuthentication(self,login,senha):
        ...
        try:
            self.cursor.execute(f"""SELECT {self.usuario},{self.senha} from {self.schema}.{self.table} where {self.usuario} = {login} and {self.senha} = '{senha}'""")
    ...
    def insertTableFun(self,cpf,nome,sobrenome,senha,cargo,salario,telefone,rua,numero,bairro,cidade,uf,cep) -> None:
        #colocar tudo numa variavel, passar por ela vendo quem é vazio e deixar NULL
        try:
            self.cursor.execute(f"""INSERT INTO padaria.endereco VALUES ({bairro},{rua},{numero},{cidade},{uf},{cep});""")
            self.cursor.execute(f"""INSERT INTO padaria.funcionario VALUES ({cpf},{nome},{sobrenome},{senha},{cargo},{salario},{telefone}, NULL);""")
            self.cursor.commit()
    ...
    def showTableFun(self,tableWidget):
        ...
        try:
            ...
            for i in range(0, len(dados_lidos)):
                for j in range(0, 12):
                    table.setItem(i,j,QtWidgets.QTableWidgetItem(str(dados_lidos[i][j])))
        except Exception as error:
            self.printError(error)
...
if __name__ == '__main__':
    ...
    a.Login()
    a.LoginAuthentication('12345678901','senha123')
