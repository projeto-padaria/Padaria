...
class Connect:
    ...
    def menu(self) -> None:
        ...
        match user_choice:
            case '1':
                return self.selectTable(self.cursor)
            case '0':
                print('Finalizando...')
                wait(5)
            case _:
                self.printError('Opção inválida')
                self.tryAgain(lambda: self.menu(self.cursor))

    def selectTable(self) -> None:
        clearTerminal('cls')
        try:
    ...
    def selectQuery(self) -> None:
        ...
        try:
            ...
            for query in self.cursor.columns(table=self.table):
    ...
    def execute(self) -> None:
        ...
        try:
            ...
            for row in self.cursor.execute(f'SELECT {self.query} FROM {self.schema}.{self.table}'):
                print(row)
